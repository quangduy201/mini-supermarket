...
import mini_supermarket.GUI.component.Statisic.blankchart.BlankPlotChart;
import mini_supermarket.GUI.component.Statisic.blankchart.BlankPlotChatRender;
...
public class CurveLineChart extends JComponent {
        ...
        blankPlotChart.setBlankPlotChatRender(new BlankPlotChatRender() {
            ...
            public void renderGraphics(BlankPlotChart chart, Graphics2D g2, Rectangle2D rectangle) {
                if (!model.isEmpty() && animate > 0 && index >= 0 && index <= legends.size() - 1) {
                    draw(g2, rectangle, 0, chart.getNiceScale().getTickSpacing() * chart.getNiceScale().getMaxTicks());
                    draw(g2, rectangle, 1, chart.getNiceScale().getTickSpacing() * chart.getNiceScale().getMaxTicks());
                    draw(g2, rectangle, 2, chart.getNiceScale().getTickSpacing() * chart.getNiceScale().getMaxTicks());
                }
            }
    ...
    private void draw(Graphics2D g2, Rectangle2D rec, int index, double maxValue) {
        ...
        float size = 6;
        g2.setPaint(new GradientPaint((int) rec.getX(), 0, legends.get(index).getColor1(), (int) (rec.getX() + rec.getWidth()), 0, legends.get(index).getColor1()));
